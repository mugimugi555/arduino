...
arduino-cli lib install "ESP8266WebServer"        # ESP8266上でWebサーバー機能を実装するためのライブラリ。HTTPリクエストの処理やWebページの提供が可能です。
arduino-cli lib install "ArduinoJson"             # JSON形式のデータを簡単に作成、解析するためのライブラリ。API通信やデータの保存に役立ちます。
arduino-cli lib install "Adafruit BME280 Library" # Adafruit社のBME280センサー用ライブラリ。温度、湿度、気圧の取得をサポートします。
...
#include <Wire.h>                 // I2C通信を使用するための標準ライブラリ。BME280などのI2Cデバイスと通信するために必要です。
#include <Adafruit_Sensor.h>      // Adafruit社製センサー用の統一インターフェースライブラリ。各種センサーからのデータ取得に役立ちます。
#include <Adafruit_BME280.h>      // BME280センサーのためのライブラリ。温度、湿度、気圧のデータ取得が簡単に行えます。
