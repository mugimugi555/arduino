...
const long jsonInterval = 6000; // JSONレスポンスを取得する間隔（6秒）
const long mdnsInterval = 1000; // mDNS更新の間隔（1秒）
...
void loop() {
    ...
    // mDNSサービスを毎秒更新
    if (currentMillis - previousMillis >= mdnsInterval) {
        previousMillis = currentMillis;
        MDNS.update(); // mDNSの更新
    }

    // ハードウェア情報を取得してJSONレスポンスを生成
    if (currentMillis - previousMillis >= jsonInterval) {
        previousMillis = currentMillis;
